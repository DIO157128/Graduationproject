...
def checkout(tool: str, project: str, bug_id: int, outputDir: str):
    ...
    if tool.lower() == 'defects4j':
        subprocess.call(
            ["defects4j", "checkout", "-p", project, '-v', str(bug_id) + 'b', '-w', './' + outputDir + '/' + tag],
    ...
    def get_original_file(project_path: str, patched_file: str):
        ...
        if tag not in path_mapping:
            ...
            for file_path in file_paths:
                curTag = project_name + '_' + os.path.basename(os.path.dirname(file_path)) + '_' + os.path.basename(
                    file_path)
...
def convert2unix(file_path: str) -> bool:
    command_result = subprocess.run(["dos2unix", file_path], encoding="utf-8", stdout=subprocess.PIPE,
...
def save_methods(buggy_methods: List[str], patched_methods: List[str], output_dir: str):
    ...
    tail = "}"
    with open(output_dir + '/' + 'buggy.java', 'w', encoding='utf8') as buggy_f, open(output_dir + '/' + 'fixed.java',
                                                                                      'w',
                                                                                      encoding='utf8') as patched_f:
...
def main(patches_path: str, output_dir: str):
    ...
    for patch in tqdm(patches_file):
        try:
            ...
            project_out_path = output_dir + '/files' + '/' + tool + '/' + '_'.join(
                [tool, project, str(bug_id)]) + '/' + apr + '_' + str(patch_id)
            method_out_path = output_dir + '/methods' + '/' + tool + '/' + '_'.join(
                [tool, project, str(bug_id)]) + '/' + apr + '_' + str(patch_id)
