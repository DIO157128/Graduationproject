...
from model import ConcatenateModel
    ...
    with open(file_path, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            if 'idx' not in js:
                js['idx'] = idx
            code = js['code']
            doc_string = js['doc_string']
            ast = js['ast']
            codes.append(code)
            docs.append(doc_string)
            asts.append(ast)
    return codes,docs,asts
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        self.examples = []
        sources,labels,asts = read_examples(file_path)
        for i in tqdm(range(len(sources))):
            self.examples.append(convert_examples_to_features(sources[i], labels[i], asts[i],tokenizer, args))
        if file_type == "train":
    ...
    def __getitem__(self, i):
        return self.examples[i].input_ids, self.examples[i].input_ids.ne(0), self.examples[i].label, self.examples[
            i].decoder_input_ids,self.examples[i].ast

def custom_collate(batch):
    batch_size = len(batch)
    input_ids = [batch[i][0] for i in range(batch_size)]
    input_ids_ne = [batch[i][1] for i in range(batch_size)]
    label = [batch[i][2] for i in range(batch_size)]
    decoder_input_ids = [batch[i][3] for i in range(batch_size)]
    ast = [batch[i][4][0] for i in range(batch_size)]

    input_ids = torch.stack(input_ids, dim=0)
    input_ids_ne = torch.stack(input_ids_ne, dim=0)
    label = torch.stack(label, dim=0)
    decoder_input_ids = torch.stack(decoder_input_ids, dim=0)
    return [input_ids,input_ids_ne,label,decoder_input_ids,ast]
def convert_examples_to_features(source, label, ast,tokenizer, args):
    # encode
    ...
    label = tokenizer.encode(label, truncation=True, max_length=args.decoder_block_size, padding='max_length',
                             return_tensors='pt')
    return InputFeatures(source_ids, label, ast,decoder_input_ids)
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (input_ids, attention_mask, labels, decoder_input_ids) = [x.squeeze(1).to(args.device) for x in batch[:4]]
            ast = batch[4]
            model.train()
            # the forward function automatically creates the correct decoder_input_ids
            loss = model(input_ids=input_ids, attention_mask=attention_mask,asts= ast)
            if args.n_gpu > 1:
                loss = loss.mean()
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    if args.stage=='generate':
        ...
        df.to_csv('../data/raw_predictions/CodeT5_generate_raw_predictions.csv')
    else:
        processed_sources = []
        original_sources = df["original_source"].tolist()
        for gt, os in zip(raw_predictions, original_sources):
            test_prefix = split_string_by_braces(os)[0]
            focal_method = split_string_by_braces(os)[1]
            processed_sources.append(
                '"<FocalMethod>" ' + focal_method + '"<TestPrefix>" ' + test_prefix.replace('"<AssertPlaceHolder>"',
                                                                                            '"<AssertPlaceHolder>" /* {} */'.format(
                                                                                                gt)))
        df['processed_source'] = processed_sources
        df["generated_type"] = raw_predictions
        df["correctly_predicted"] = accuracy
        df.to_csv('../data/raw_predictions/CodeT5_type_raw_predictions.csv')
