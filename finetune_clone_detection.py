...
from model import ConcatenateModel, CloneDetectionConcatenateModel
...
def read_examples(file_path):
    ...
    with open(file_path, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            ast2s.append(ast2)
            labels.append(label)
    return code1s,code2s,ast1s,ast2s,labels
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        self.examples = []
        code1s,code2s,ast1s,ast2s,labels = read_examples(file_path)
        for i in tqdm(range(len(code1s))):
            self.examples.append(convert_examples_to_features(code1s[i],code2s[i],ast1s[i],ast2s[i],labels[i],tokenizer, args))
        if file_type == "train":
            for example in self.examples[:3]:
                logger.info("*** Example ***")
                logger.info("label: {}".format(example.label))
                logger.info("code1_ids: {}".format(' '.join(map(str, example.code1_ids))))
                logger.info("code2_ids: {}".format(' '.join(map(str, example.code2_ids))))

    def __len__(self):
        return len(self.examples)

    def __getitem__(self, i):
        return self.examples[i].code1_ids, self.examples[i].code1_ids.ne(0),self.examples[i].code2_ids, self.examples[i].code2_ids.ne(0), self.examples[i].ast1,self.examples[i].ast2,self.examples[i].label

def custom_collate(batch):
    batch_size = len(batch)
    code1_ids = [batch[i][0] for i in range(batch_size)]
    code1_ne = [batch[i][1] for i in range(batch_size)]
    code2_ids = [batch[i][2] for i in range(batch_size)]
    code2_ne = [batch[i][3] for i in range(batch_size)]
    ast1 = [batch[i][4][0] for i in range(batch_size)]
    ast2 = [batch[i][5][0] for i in range(batch_size)]
    label = [batch[i][6] for i in range(batch_size)]

    code1_ids = torch.stack(code1_ids, dim=0)
    code1_ne = torch.stack(code1_ne, dim=0)
    code2_ids = torch.stack(code2_ids, dim=0)
    code2_ne = torch.stack(code2_ne, dim=0)
    label = torch.stack(label, dim=0)
    return [code1_ids,code1_ne,code2_ids,code2_ne,label,ast1,ast2]
def convert_examples_to_features(code1,code2,ast1,ast2,label,tokenizer, args):
    # encode
    code1 = code1.replace('<s>', '')
    code1 = code1.replace('</s>', '')
    code2 = code2.replace('<s>', '')
    code2 = code2.replace('</s>', '')
    code1_ids = tokenizer.encode(code1, truncation=True, max_length=args.encoder_block_size, padding='max_length',
                                  return_tensors='pt')
    code2_ids = tokenizer.encode(code2, truncation=True, max_length=args.encoder_block_size, padding='max_length',
                                 return_tensors='pt')
    label = torch.tensor(label,dtype=torch.float)
    return InputFeatures(code1_ids,code2_ids,ast1,ast2,label)
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (code1_ids,code1_ne,code2_ids,code2_ne) = [x.squeeze(1).to(args.device) for x in batch[:4]]
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for batch in bar:
        ...
        ast2 = batch[6]
        with torch.no_grad():
            out_puts = model(code1_ids, code1_ne, code2_ids, code2_ne, label,ast1,ast2,True)
            logits.append(out_puts.cpu().numpy())
            y_trues.append(label.cpu().numpy())
    logits = np.concatenate(logits, 0)
    y_trues = np.concatenate(y_trues, 0)
    y_preds = torch.sign(torch.tensor(logits,dtype=torch.float))
    ...
    f = open("./results/clone_res.txt", "a")
...
def main():
    ...
    model = CloneDetectionConcatenateModel(args)
